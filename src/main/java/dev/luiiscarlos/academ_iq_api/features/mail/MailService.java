...
public class MailService {
    ...
    public void sendMailVerification(User user, String origin) {
        String url = generateUrl(origin, "auth/verify", tokenService.generateVerificationToken(user));
        ...
        try {
            ...
            String htmlContent = mailTemplateProcessor.buildTemplate("email-template", vars);
        ...
        log.info("A verification e-mail has been sent to '%s' at %s", user.getUsername(), LocalDateTime.now());
    }
    ...
    public void sendMailPasswordRecover(User user, String origin) {
        try {
            String token = generateUrl(origin, "auth/reset-password", tokenService.generateRecoverPasswordToken(user));
            MimeMessage message = mailSender.createMimeMessage();
            MimeMessageHelper helper = new MimeMessageHelper(message, true);

            String htmlMessage = String.format("<html>" +
                "<head>" +
                    "<style>" +
                ...
                "<body>" +
                    "<div class='container'>" +
                        "<div class='logo-container'>" +
                            "<img src='https://res.cloudinary.com/duu4u98gb/image/upload/v1747091207/academiq-logo_gyofsk.png' alt='Academ-IQ Logo' class='logo'>" +
                        "</div>" +
        ...
        log.info("A recover password e-mail has been sent to '%s' at %s", user.getUsername(), LocalDateTime.now());
    }
